...
class DialpadViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private val T9 = mapOf(
        '2' to "abcABC", '3' to "defDEF", '4' to "ghiGHI", '5' to "jklJKL",
        '6' to "mnoMNO", '7' to "pqrsPQRS", '8' to "tuvTUV", '9' to "wxyzWXYZ"
    )

    init { viewModelScope.launch { allContacts = repo.getAll() } }
    ...
    private fun search() {
        ...
        viewModelScope.launch {
            _suggestions.value = allContacts.filter { c ->
                c.phoneNumbers.any { it.number.contains(q) } || matchT9(c.name, q)
            }.take(6)
        }
    }
    ...
    private fun matchT9(name: String, digits: String): Boolean {
        ...
        return digits.forEachIndexed { i, d ->
            val l = letters.getOrNull(i) ?: return false
            if (l !in (T9[d] ?: return false)) return false
        }.let { true }
    }
}
...
class SuggestionsAdapter(private val onClick: (Contact) -> Unit)
    ...
    inner class VH(private val b: ItemSuggestionBinding) : RecyclerView.ViewHolder(b.root) {
        fun bind(c: Contact) {
            ...
            b.tvNumber.text = c.phoneNumbers.firstOrNull()?.number ?: ""
            b.tvType.text = c.phoneNumbers.firstOrNull()?.type ?: ""
    ...
    override fun onCreateViewHolder(p: ViewGroup, t: Int) = VH(
        ItemSuggestionBinding.inflate(LayoutInflater.from(p.context), p, false)
    )
...
class DialpadFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val adapter = SuggestionsAdapter { contact ->
            contact.phoneNumbers.firstOrNull()?.let { (activity as? MainActivity)?.call(it.number) }
        }
        ...
        mapOf(
            ...
            b.btn8 to "8", b.btn9 to "9", b.btnStar to "*", b.btnHash to "#"
        ).forEach { (btn, d) ->
        ...
        b.btnCall.setOnClickListener {
            vm.number.value?.takeIf { it.isNotBlank() }?.let { (activity as? MainActivity)?.call(it) }
        }
        b.btnSms.setOnClickListener {
            vm.number.value?.takeIf { it.isNotBlank() }?.let { (activity as? MainActivity)?.sms(it) }
        }
    ...
    private fun formatNumber(n: String): String = when {
        n.startsWith("+") || n.length <= 4 -> n
        n.length == 10 -> n.chunked(2).joinToString(" ")
        else -> n
    }
